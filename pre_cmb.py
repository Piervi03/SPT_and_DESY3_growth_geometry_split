...
GEO/GROWTH SPLIT:
  ...
  - D_M(z_star)           : late-time projection distance. Computed with
...
rather than reusing cosmo.dA/cosmo.Ez. 

Assumes flat geometry (omega_k=0)
...
def _omega_r(h0, tcmb, neff,neutrinos_bool):
    ...
    omega_nu_h2 = 0.2271 * neff * omega_gamma_h2*neutrinos_bool
    omega_r_h2 = omega_gamma_h2 + omega_nu_h2
    return omega_r_h2 / h0 ** 2


def _z_star_hu_sugiyama(ombh2, omch2, omnuh2,block):
    ...
    z_star = 1048.0 * (1.0 + 0.00124 * ombh2 ** -0.738) * (1.0 + g1 * om_geo_h2 ** g2)
    return z_star


def _sound_horizon_eh98(ombh2, omch2, omnuh2, tcmb, z_target,block):
    """Eisenstein & Hu (1998) closed-form sound horizon integral,
    evaluated out to an arbitrary target redshift (z_drag for BAO, or
    z_star here for the CMB). """
    om_h2 = ombh2 + omch2 + omnuh2
    theta27 = tcmb / 2.7
    om_geo_h2 = block['cosmological_parameters', 'Omega_m_geo']*block['cosmological_parameters', 'h0']**2
...
def execute(block, config):
    ...
    # --- z_star and r_s(z_star)  ---
    z_star = _z_star_hu_sugiyama(ombh2, omch2, omnuh2,block)
    r_s_star = _sound_horizon_eh98(ombh2, omch2, omnuh2, tcmb, z_star,block)
    ...
    omega_r = _omega_r(h0, tcmb, neff,neutrinos_bool)
