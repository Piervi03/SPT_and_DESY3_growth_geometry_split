...
class DistCompute:
    ...
    def run(self, HMF, cosmology, scaling):
        ...
        self.richness_m = scaling_relations.mass2obs('richness', self.HMF['M_arr'][None,:], self.HMF['z_arr'][:,None], self.scaling)
    ...
    def run_field(self, fieldidx):
        ...
        # Cut in zeta
        dN_dz_dlnobs[(this_zeta_m[:,None,:]*np.ones(dN_dz_dlnobs.shape)<self.scaling['zeta_min'])] = 0
        # P(xi>cut)
        ...
        with np.errstate(divide='ignore'):
            lndN_dz_dlnrichness_grid = np.array([np.interp(self.lnlambda_bins, np.log(self.richness_m[i]), lndN_dz_dlnrichness[i]) for i in range(len(self.HMF['z_arr']))])

        # dN/dz
        dN_dz = np.sum(np.exp(.5*(lndN_dz_dlnrichness[:,1:]+lndN_dz_dlnrichness[:,:-1])) * np.diff(np.log(self.richness_m)), axis=1)
