...
# Syntax
# python SPT_NC_with_DES_WL_split_dev/mock_WL.py SPT_NC_with_DES_WL_split_dev/WL_input.py split_analysis/mockinput_split_10_par.py split_analysis/mock_cat_split_10_par.fits 
...
def main():
    ...
    cosmology_growth = mockconfigMod.cosmology_growth
    cosmology_geo = mockconfigMod.cosmology_geo
    cosmology_both = mockconfigMod.cosmology_both
    cat = Table.read(sys.argv[3])
    
    # DES weak lensing
    mock_WL = MockUpDESWL(cosmology_both, sys.argv[1]) 
...
class MockUpDESWL:
    ...
    def __init__(self, cosmology_both, WLconfigname):
        self.cosmology_both = cosmology_both
        ...
        self.MCrel = Mconversion_concentration.ConcentrationConversion(self.config_mod.DES['mcType'], cosmology_both, setup_interp=True)
    ...
    def get_gt(self, z, beta_bin, w_r_bin):
        ...
        R_mis = self.miscenterer.get_mean_Rmis(self.cat, self.cosmology_both)
    ...
    def get_beta(self, z_cl):
        ...
        beta = np.array([cosmo.dA_two_z(z_cl, z, self.cosmology_both)/cosmo.dA(z, self.cosmology_both) for z in self.source_z['z']])
    ...
    def __call__(self, cat):
        ...
        self.rho_c_z = cosmo.RHOCRIT * cosmo.Ez(z_cl, self.cosmology_both)**2
        self.Dl = cosmo.dA(z_cl, self.cosmology_both)
