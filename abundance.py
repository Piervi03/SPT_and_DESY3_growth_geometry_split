...
class NumberCount:
    ...
    def lnlike(self, block):
        ...
        for fieldidx,field in enumerate(self.SPTfieldNames):
            ...
            lndNdxi = interpolate.interp2d(np.log(self.xi_bins), np.log(HMF['z_arr'][1:]), np.log(dN_dxi[1:,:]), kind='cubic')

            # Ntotal
            dNdz = np.array([np.sum(np.exp(.5*(lndNdxi(np.log(self.xi_arr[1:]), np.log(z)) + lndNdxi(np.log(self.xi_arr[:-1]), np.log(z)))) * (self.xi_arr[1:]-self.xi_arr[:-1])) for z in self.z_arr])
            Ntotal+= np.trapz(dNdz, self.z_arr)

            ##### cluster contributions
            thisfield_conf = np.where((self.catalog['field']==field)
            ...
            for i in thisfield_conf:
                ...
                if self.catalog['redshift_err'][i]==0.:
                    thislnlike = lndNdxi(np.log(self.catalog['xi'][i]), np.log(self.catalog['redshift'][i]))

                ##### photo-z
                elif self.catalog['redshift_err'][i]>0.:
                    zlo = min((.25, self.catalog['redshift'][i]-4*self.catalog['redshift_err'][i]))
                    zhi = max((HMF['z_arr'][-1], self.catalog['redshift'][i]+4*self.catalog['redshift_err'][i]))
            ...
            thisfield_unconf = np.where((self.catalog['field']==field) & (self.catalog['xi']>=self.surveyCutSZ[0]) & (self.catalog['xi']<=self.surveyCutSZ[1]) & (self.catalog['redshift']==0.) & (self.catalog['redshift_lim']<=self.surveyCutRedshift[1]))[0]
            for i in thisfield_unconf:
                dNdxifalse = self.SPTnFalse_beta[fieldidx] * self.SPTfieldSize[fieldidx]/2500 * self.SPTnFalse_alpha[fieldidx] * np.exp(-self.SPTnFalse_beta[fieldidx]*(self.catalog['xi'][i]-5.))
                zarr = np.linspace(self.catalog['redshift_lim'][i], HMF['z_arr'][-1], 25)
                thislnlike = np.log(dNdxifalse + np.trapz(np.exp(lndNdxi(np.log(self.catalog['xi'][i]), np.log(zarr))[:,0]), zarr))
