...
functions = {
    'theta_to_H0': theta_to_H0_interface,
    'H0_to_theta': H0_to_theta_interface,
    'sqrt': sqrt,
    'log': log,
    'exp': exp,
    'pow': pow,
...
NON_PARAMETER_NAMES = frozenset(functions) | frozenset(["locals", "True", "False", "None", "nan"])


def relation_parameter_names(expression):
    try:
...
def find_relation_parameters(relations):
    names = set()
    for name, function in relations:
        names.add(name)
        names.update(relation_parameter_names(function))
    names.difference_update(NON_PARAMETER_NAMES)
    return names
...
COSMOLOGY_CONSISTENCY_RELATIONS = [
    ("omega_m_geo", "omgeoh2/h0/h0"),
    ("omgeoh2",        "omega_m_geo*h0*h0"), 
    ("omega_m_growth", "omgrowthh2/h0/h0"),
    ("omgrowthh2",     "omega_m_growth*h0*h0"),
    ("omega_b", "ombh2/h0/h0"),
    ("omega_c", "omch2/h0/h0"),
    ...
    ("omch2", "omgrowthh2-ombh2-omnuh2"),
    ("omgrowthh2", "omch2+ombh2+omnuh2"),
    ("omnuh2", "omgrowthh2-ombh2-omch2"),
    
    ("baryon_fraction", "omega_b/omega_m_growth"),
    ("omega_b", "omega_m_growth*baryon_fraction"),
    ("omega_m_growth", "omega_b/baryon_fraction"),
    ("baryon_fraction", "ombh2/omgrowthh2"),
    ("ombh2", "omgrowthh2*baryon_fraction"),
    ("omgrowthh2", "ombh2/baryon_fraction"),
    
    ("omega_m_growth", "omega_b+omega_c+omega_nu"),
    ("omega_b", "omega_m_growth-omega_c-omega_nu"),
    ("omega_c", "omega_m_growth-omega_b-omega_nu"),
    ("omega_nu", "omega_m_growth-omega_b-omega_c"),
    
    ("h0", "(omgeoh2/omega_m_geo)**0.5"),
    ("h0", "(omgrowthh2/omega_m_growth)**0.5"),
    ...
    ("omega_lambda", "1-omega_m_geo-omega_k"),
    ("omega_m_geo", "1-omega_lambda-omega_k"),
    ("omega_k", "1-omega_m_geo-omega_lambda"),
...
class Consistency(object):
    def __init__(self, relations, possible_defaults, verbose=False, extra_fixed=None):
        ...
        self.target_parameters = list(dict.fromkeys(name for name, function in relations))
        self.parameter_names = find_relation_parameters(relations)
        self.parameters = {}
    ...
    def blank_parameters(self):
        return {name: nan for name in self.parameter_names}
    ...
    def reset(self):
        ...
        # unspecified
        self.parameters = self.blank_parameters()
